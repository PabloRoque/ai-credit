...
[![npm version](https://img.shields.io/npm/v/ai-contrib.svg)](https://www.npmjs.com/package/ai-contrib)
...
npx ai-contrib /path/to/your/repo

# Export as JSON
npx ai-contrib -f json -o report.json

# Only analyze specific tools
npx ai-contrib -t claude,codex
...
```bash
npx ai-contrib files [path] [-n LIMIT]
```
...
```bash
npx ai-contrib history [path] [-n LIMIT]
```
...
```bash
npx ai-contrib sessions [path] [-t TOOL]
```
